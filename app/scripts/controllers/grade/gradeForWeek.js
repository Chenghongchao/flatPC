...
function($scope,AppConfig,$rootScope,FlatService,TermService,$filter,GradeService,RoomService,PublicService,RuleService) {
            ...
            if(!rule.isconfiscat){
                ...
                function(confirm){
                    ...
                    RuleService.checkByRoom({
                        ...
                        schoolcode:AppConfig.schoolCode,
                        roomid: that.item.roomId,
                        specialid: that.item.roomId+'-'+$rootScope.treeTerm[$scope.media.yearIndex].semesterList[$scope.media.termIndex].semesterId+'-'+$scope.media.week,
    ...
    $scope.subImport = function(fun){
        ...
        $rootScope.loading = true;
        return GradeService.importScoresWeek(fdata,$scope.type).success(function(data){
            //console.log(data);
            if(data.code == 0){
