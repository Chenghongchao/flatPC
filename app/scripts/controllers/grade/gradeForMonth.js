...
.controller('GradeForMonthCtrl', ['$scope','AppConfig','$rootScope', 'FlatService','TermService','$filter','GradeService','RoomService','PublicService','RuleService',
function($scope,AppConfig,$rootScope,FlatService,TermService,$filter,GradeService,RoomService,PublicService,RuleService) {
    ...
    $scope.media = {
        ...
        weekList:[],
        now:function (week) {
            if(new Date(week.year + '-' + week.month + '-01').Format('yyyy-MM-dd') == new Date(new Date().Format('yyyy-MM') + '-01').Format('yyyy-MM-dd')){
                return true
            }else return false;
        },
        ...
        setYear:function (n) {
            ...
            try{
                this.weekList = $filter('sliceMonth')($rootScope.treeTerm[$scope.media.yearIndex].semesterList[$scope.media.termIndex]);
                this.week = this.weekList[0][0] || null;
            }catch(e){
                this.weekList = $filter('sliceMonth')($rootScope.treeTerm[$scope.media.yearIndex].semesterList[$scope.media.termIndex]);
                this.week = {month:new Date().getMonth()+1,year:new Date().getFullYear()};
            }
    ...
    $scope.cardMedia = {
        ...
        setTab:function (n) {
            ...
            switch(n){
                ...
                case 2:
                    if($scope.switch.bed && $rootScope.menuCheck(294)){
                        this.tab = n;
                        if(this.bed){
                            return null;
                        }
                ...
                case 3:
                    if($scope.switch.photo && $rootScope.menuCheck(295)){
                        this.tab = n;
                        if(this.img){
                            return null;
                        }
                ...
                case 4:
                    if($scope.switch.role && $rootScope.menuCheck(297)){
                        this.tab = n;
                        if(this.rule){
                            return null;
                        }
            ...
            switch (this.tab) {
                case 1:
                    if(this.item.roomScoreId){
                        ...
                        if($rootScope.treeMonth[0] && $rootScope.treeMonth[0].typeList && $rootScope.treeMonth[0].typeList[0].itemList && $scope.media.tab==1)
                        {
                            that.room = $rootScope.treeMonth[0].typeList[0].itemList;
                            that.getSum(true);
                            that.getBedData();
                            //$rootScope.treeMonth[0].tableId;
                            //$rootScope.treeMonth[0].typeList[0].typeId;
                        }    
                        else
                            that.room = [];
                        
                        //console.log($rootScope.treeMonth[0]);
        ...
        getListByRoom: function(){
            ...
            return RuleService.getListByRoom({
                ...
                specialid:this.item.roomId+'-'+new Date($scope.media.week.year + '-' + $scope.media.week.month + '-01').Format('yyyy-MM')
            }).success(function (data) {
                $rootScope.loading = false;
                    ...
                    if(data.data.length>0 && $scope.media.tab==1){
                        that.bed.forEach(function (bed) {
                            if(bed.itemList.length < 1){
                                ...
                                var options = [];
                                if($rootScope.treeMonth[0] && $rootScope.treeMonth[0].typeList && $rootScope.treeMonth[0].typeList[0].itemList)
                                {
                                    options = $rootScope.treeMonth[0].typeList[1].itemList;
                                    //$rootScope.treeMonth[0].tableId;
                                    //$rootScope.treeMonth[0].typeList[1].typeId;
                                }    
                                else
                                    options = [];
                                    // //console.log(options);
            ...
            if(!rule.isconfiscat){
                ...
                function(confirm){
                    ...
                    RuleService.checkByRoom({
                        ...
                        specialid:that.item.roomId+'-'+new Date($scope.media.week.year + '-' + $scope.media.week.month + '-01').Format('yyyy-MM'),
        ...
        roomGrade:function(fun){
            ...
            if(this.item.roomScoreId){
                    ...
                    GradeService.setGrade({
                        ...
                        adminid:AppConfig.adminId,
                        scoreitem:grades,
                        typeid:$rootScope.treeMonth[0].typeList[0].typeId,
                        tableid:$rootScope.treeMonth[0].tableId,
                        mold:2,
                        type: that.roomGradeType ? that.roomGradeType : null,
                        score: that.roomGradeType ? that.score : null,
                    }).success(function(data){
            ...
            if(grades.length > 0){
                ...
                if(that.bedGradeType!=-1){
                    GradeService.editBedGrade({
                        ...
                        date:new Date($scope.media.week.year + '-' + $scope.media.week.month + '-01').Format('yyyy-MM'),
                        scoreitem:grades,
                        typeid:$rootScope.treeMonth[0].typeList[1].typeId,
                        tableid:$rootScope.treeMonth[0].tableId,
    ...
    function getSetting() {
        if(!$rootScope.treeMonth)
            return GradeService.getSettingList({type:2,isopen:1}).success(function(data){
                if(data.code == 0){
                    $rootScope.treeMonth = data.data;
                    getRule();
                }else if(data.code == 4037){
                            swal("提示","错误代码："+ data.code + '，' + data.msg, "warning"); 
                            location.href="#login";$rootScope.loading = false;
        ...
        for(var i = 0;i < $rootScope.treeTerm.length;i ++){
            for(var j = 0; j < $rootScope.treeTerm[i].semesterList.length ; j ++ ){
                if(($rootScope.treeTerm[i].semesterList[j].isCurrent) || (i == $rootScope.treeTerm.length-1 && j == $rootScope.treeTerm[i].semesterList.length-1)){
                    ...
                    var start = new Date($rootScope.treeTerm[i].semesterList[j].startTime),end = new Date($rootScope.treeTerm[i].semesterList[j].endTime),now = new Date();
                    ...
                    $scope.media.weekList  = $filter('sliceMonth')($rootScope.treeTerm[$scope.media.yearIndex].semesterList[$scope.media.termIndex]);
                    // //console.log($scope.media.weekList);
                    if($rootScope.menuCheck(283)){
                        $scope.media.tab = 1;
                    }else if($rootScope.menuCheck(284)){
                        $scope.media.tab = 2;
                    }else if($rootScope.menuCheck(285)){
                        $scope.media.tab = 3;
                    }else{
                        return;
                    }
    ...
    function refresh(n) {
        if($scope.media.flatid.length<1 || !$scope.media.week || !$scope.media.week)return;
        ...
        swal({   
            ...
            function(){
                ...
                return GradeService.allQuickScores({
                    ...
                    date: new Date($scope.media.week.year + '-' + $scope.media.week.month + '-01').Format('yyyy-MM'),
                    adminid:AppConfig.adminId,
                    roomtypeid: $rootScope.treeMonth[0].typeList[0].typeId,
                    bedtypeid: $rootScope.treeMonth[0].typeList[1].typeId,
    ...
    $scope.subImport = function(fun){
        ...
        fdata.append('token',AppConfig.token);
        fdata.append('schoolcode',AppConfig.schoolCode);
        fdata.append('roomtypeid',$rootScope.treeMonth[0].typeList[0].typeId);
        fdata.append('bedtypeid',$rootScope.treeMonth[0].typeList[1].typeId);
        fdata.append('adminid',AppConfig.adminId);
        // console.log(uploadExcel);
        $rootScope.loading = true;
        return GradeService.importScoresMonth(fdata,$scope.type).success(function(data){
            //console.log(data);
            if(data.code == 0){
